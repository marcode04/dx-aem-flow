...
| Severity | Confidence | PR Impact | Examples |
|----------|-----------|-----------|----------|
| **Critical** (must fix) | 90-100 | Blocks approval — request changes | Security vuln, broken functionality, data loss |
| **Important** (should fix) | 80-89 | Include in review — fix before merge | Logic error, missing error handling, performance |
| **Suggestion** (consider) | 70-79 | Dropped by default (below 80 threshold) | Clarity, naming, minor optimization |

**Workflow semantics:**
- If ANY Critical finding exists → verdict MUST be "Changes requested"
- If only Important findings → verdict is "Approved with suggestions" (author should fix before merge)
- If no findings ≥ 80 → verdict is "Approved"
