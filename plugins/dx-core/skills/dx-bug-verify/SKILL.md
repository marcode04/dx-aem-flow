---
name: dx-bug-verify
description: Reproduce a bug using Chrome DevTools — navigate to the repro URL, follow repro steps, take screenshots, and confirm whether the bug is reproducible. Supports `before` (default) and `after` modes. Works with Azure DevOps/Jira. Use after /dx-bug-triage or after /dx-bug-fix (with `after` argument) to verify the fix on local AEM.
argument-hint: "<work-item-id or issue-key> [before|after]  (default: before)"
...
Examples: `2453532`, `2453532 before`, `2453532 after`, `after` (uses most recent spec).

Set `MODE` to `before` or `after`. This affects URL selection, output file, screenshot prefix, result interpretation, and ADO comment.

| Aspect | `before` | `after` |
|--------|----------|---------|
| URL source | Repro URL from `raw-bug.md` (QA/remote) | Same page path on local AEM (`aem.author-url` from `.ai/config.yaml`) |
| Output file | `verification.md` | `verification-local.md` |
| Screenshot prefix | `step-` | `post-fix-step-` |
| Success = | `Reproduced` (bug confirmed) | `Fix Verified` (bug no longer present) |
| Failure = | `Could Not Reproduce` | `Fix Failed` (bug still present) |
| ADO comment tag | `[BugVerify]` | `[BugVerifyLocal]` |
...
3. **Compare:**
   ...
   - **`after` mode:** Does the observed state match the "Expected Behavior" (correct behavior) described in raw-bug.md?
...
Save to `$OUTPUT_FILE` (`verification.md` for `before`, `verification-local.md` for `after`).
